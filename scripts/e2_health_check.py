...
def health_summary(hours: int = 24) -> str:
    ...
    expected_per_type = len(SYMBOLS) * hours
    lines = [
    ...
    for data_type, folder in DATA_TYPES.items():
        ...
        missing_symbols = [s for s in SYMBOLS if by_symbol.get(s, 0) == 0]
        coverage = len(success) / expected_per_type if expected_per_type else 0.0
        ...
        lines.extend([
            ...
            f"  rows_collected_last_{hours}h: {len(success)}",
    ...
    lines.extend([
        ...
        f"api_error_count: {total_errors}",
    ])
    return "\n".join(lines) + "\n"
